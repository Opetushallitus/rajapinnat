...
public class TiedonSiirtoToinenAsteServiceImplTest {
    ...
    private static final String HETU_EXCEPTION = "151093-9712";
    ...
    public void importArvosanat() throws Exception {
        ...
        TiedonSiirtoToinenAsteServiceImpl service = getTiedonSiirtoToinenAsteServiceMock();
        ...
        // * arvosana henkilölle, joka on olemassa -> OK
        ...
        rowset.getROW().add(TiedonSiirtoTstUtils.createSampleRowArvosana(HETU_EXCEPTION));
        ...
        assertEquals(HETU_EXCEPTION, response.getErrorRow().get(1).getHetu());
        assertEquals("HETU_EXCEPTION", response.getErrorRow().get(1).getErrorMessage());
    ...
    @Test
    public void importHenkilotiedot() throws Exception {
        // luodaan service jossa muut palvelut mockattu
        TiedonSiirtoToinenAsteServiceImpl service = getTiedonSiirtoToinenAsteServiceMock();

        // kutsutaan importtia, jossa seuraavanlaista dataa
        // * henkilö, joka heittää poikkeuksen (esim joku integraatio ei onnistu)
        // * henkilö, jonka import OK
        // * henkilö, joka on jo olemassa
        fi.vm.sade.henkilo.service.types.perusopetus.henkilotiedot.ROWSET rowset = new fi.vm.sade.henkilo.service.types.perusopetus.henkilotiedot.ROWSET();
        rowset.getROW().add(TiedonSiirtoTstUtils.createSampleRowHlotiedot(HETU_EXCEPTION));
        rowset.getROW().add(TiedonSiirtoTstUtils.createSampleRowHlotiedot(HETU_OID_NULL)); // oid null koska hlöä ei vielä kuulu olla
        rowset.getROW().add(TiedonSiirtoTstUtils.createSampleRowHlotiedot(HETU_OID_FOUND));
        GenericResponse response = service.importHenkilotiedot(rowset);

        // assertoidaan, että responsesta löytyy onnistuneet ja epäonnistuneet rivit
        // ...assertoidaan responsen yleiset tiedot
        assertEquals(1, response.getSuccessRowCount().intValue());
        assertEquals(2, response.getErrorRowCount().intValue());
        assertEquals("OK with 2 errors", response.getResponseMessage());
        // ...assertoidaan responsen virherivit
        assertEquals(HETU_EXCEPTION, response.getErrorRow().get(0).getHetu());
        assertEquals("HETU_EXCEPTION", response.getErrorRow().get(0).getErrorMessage());
        assertEquals(HETU_OID_FOUND, response.getErrorRow().get(1).getHetu());
        assertEquals("henkilo already exists: "+HETU_OID_FOUND, response.getErrorRow().get(1).getErrorMessage());
    }

    @Test
    public void validateAndImportArvosanatXml_reallyValidates() throws Exception {
        TiedonSiirtoToinenAsteServiceImpl service = new TiedonSiirtoToinenAsteServiceImpl();
        String xml = IOUtils.toString(Thread.currentThread().getContextClassLoader().getResourceAsStream("toinenaste/perusopetusArvosanat.xml"));
        try {
            service.validateAndImportArvosanatXml(new ByteArrayInputStream(xml.replace("2013", "2013x").getBytes())); // call with invalid year
            fail();
        } catch (Exception e) {
            assertTrue(e.toString().contains("Value '2013x' is not facet-valid"));
        }
    }

    @Test
    public void validateAndImportHenkilotiedotXml_reallyValidates() throws Exception {
        TiedonSiirtoToinenAsteServiceImpl service = new TiedonSiirtoToinenAsteServiceImpl();
        String xml = IOUtils.toString(Thread.currentThread().getContextClassLoader().getResourceAsStream("toinenaste/perusopetusHenkilotiedot.xml"));
        try {
            service.validateAndImportHenkilotiedotXml(new ByteArrayInputStream(xml.replace("2013", "2013x").getBytes())); // call with invalid year
            fail();
        } catch (Exception e) {
            assertTrue(e.toString().contains("Value '2013x' is not facet-valid"));
        }
    ...
    private TiedonSiirtoToinenAsteServiceImpl getTiedonSiirtoToinenAsteServiceMock() {
        TiedonSiirtoToinenAsteServiceImpl service = new TiedonSiirtoToinenAsteServiceImpl();
